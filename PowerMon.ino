...
//#define   TRACING                           // print on serial the debug msg
...
#define   EEMCLR        0x40                  // Month clear (Monx = index)
#define   EEGCLR        0x80                  // Globally Clear


// For debug only (show ALL cmd sent to eeprom) NOTE: MUST be in one line 
#ifdef    TRACING
 #define  PR         Serial.printf 
 #else
  #define  PR       // 
#endif
...
typedef struct {
                ...
                word            Flags,_w;     // general flags, filler
                TMcp39F511Data  Mcp;          // Mcp Data block (68)
                word            RSSI;         // wifi signal
                word            Wmax[2];      // peak power           (clearable)
                word            Time[2];      // peak power timestamp (clearable)
                word            Elaps[2];     // Time elapsed         (clearable)
                byte            Cmd,_b;       // copy of cmd callback
                dword           Whi[2];       // energy in            (clearable)
                dword           Who[2];       // energy out           (clearable)
                byte            Misc[12];     // Misc buffer to reach 128 bytes
               } TRtData;                     // sizeof(TRtData) = 128 bytes

typedef struct {
                byte            Mon,Day;      // Creation date
                word            Vmin,Vmax;    // 
                word            Wmax[2];      // max power  (copy of Mcp @ saveminmax)
                word            Time[4];      // time when Vmin/max occour + Wmax[] (copy of Mcp @ saveminmax)
...
char            NewHost[16],HostChg;          // new host name and sequencer to save and reboot
...
//     PR("fmt (%4hx ->",addr);
     // 0xff fill remaining pages
     for (byte lp =0; lp < 31 * 4; lp++)      // loop for 31 days
        {
         // Zero fill 1st page (+00.. +0x7f)
         Eep.PgFill(addr >> 7,0); addr += 128;
         Eep.PgErase(addr >> 7);  addr += 128;
...
if (p->Cmd & EEWRCR)
    ...
    // -----------------------------------
    {
     ...
     memset(&Data.Mcp,0, sizeof(Data.Mcp)); Data.Mcp.Vaw.Vmin = 9999;
     // reset clearable      
     Data.Wmax[0] = Data.Time[0] = Data.Elaps[0] = 0;
     Data.Wmax[1] = Data.Time[1] = Data.Elaps[1] = 0;

//     PR("DyCreate: %hhd-%hhd (%4hx)\n",Log.Day,Log.Mon, addr); 
     p->Cmd &= ~EEWRCR;       
    }
...
//     PR("Wr:[%2hhd.%2hhd][%2hhd] (%04hx) %5hd %5hd\n",p->Monx, p->LDay, p->Idx, p->Addr,d[0],d[1]);
     p->Cmd &= ~EEWRPW;       
    }

 if (p->Cmd & EEDYRD)
    // -----------------------------------
    // read a the selected day (.Addr)
    // and copy on runtime log 
    // (will be restored at 1st 5 min)
    // INPUT:
    //        .Addr
    // -----------------------------------
    {
...
// -----------------------------------------
{
  ...
  Data.Mcp.Hour = Data.Clk.Hour; Data.Mcp.Min = Data.Clk.Min;
 #else
    // Get MCP data
    Data.Mcp.Hour = Data.Clk.Hour; Data.Mcp.Min = Data.Clk.Min;
...
// Execute CmdCallBack Cmd (copied from callback)
 if (Data.Cmd == 1)         // reset #1
    {Data.Wmax[0] = Data.Time[0] = Data.Elaps[0] = 0; lwi[0] = Data.Mcp.Wh.Pi[0]; lwo[0] = Data.Mcp.Wh.Po[0]; Data.Cmd = 0;}
  else
   if (Data.Cmd == 2)       // reset #2
      {Data.Wmax[1] = Data.Time[1] = Data.Elaps[1] = 0; lwi[1] = Data.Mcp.Wh.Pi[1]; lwo[1] = Data.Mcp.Wh.Po[1]; Data.Cmd = 0;}
  
  // calc actual Whi/Who (clearable)      
  Data.Whi[0] = Data.Mcp.Wh.Pi[0] - lwi[0]; Data.Who[0] = Data.Mcp.Wh.Po[0] - lwo[0]; 
  Data.Whi[1] = Data.Mcp.Wh.Pi[1] - lwi[1]; Data.Who[1] = Data.Mcp.Wh.Po[1] - lwo[1]; 
  ...
  // at boot save 1st valid value read
  if (!lwh[0])
    {
      lwh[0] = (int)(Data.Mcp.Wh.Pi[0] - Data.Mcp.Wh.Po[0]);
      lwh[1] = (int)(Data.Mcp.Wh.Pi[1] - Data.Mcp.Wh.Po[1]);
    }
  ...
  // Update Log with runtime data if selected actual day
  if ((MIdx == Data.Clk.Mon) && (DIdx == Data.Clk.Day))
    {
     Log.Vmin     = Data.Mcp.Vaw.Vmin;
     Log.Vmax     = Data.Mcp.Vaw.Vmax;
     Log.Wmax[0]  = Data.Mcp.Vaw.Wmax[0];
  ...
  Data.RSSI = WiFi.RSSI();
  // TEMP debugger
  Data.Misc[0] = idx; Data.Misc[1] = lidx; Data.Misc[2] = lday; Data.Misc[3] = mon; Data.Misc[4] = lmon;
...
bool CmdCallBack(byte code, void *txt)
// ------------------------------------
 ...
 switch (code)
    {
      ...
      case  2:  // Reset Power #2
                Data.Cmd = code;          
                break;
      case  3:  // Totally Reset EEprom data
                EeCmd.Cmd |= EEGCLR;
                break;
      case  10: // set log index
                word v;  
                if (sscanf((char*)txt,"%hd",&v))
                  { 
                  MIdx = v >> 8; DIdx = v & 0x7f;
//                  PR("M%02hhd:D%02hhd\n",MIdx,DIdx);
                  EeCmd.Addr = DAYADDR + MONSIZE * (((word) MIdx -1) & 0x3) + DAYSIZE * ((word)DIdx -1); 
                  EeCmd.Data = &Log; 
                  EeCmd.Cmd |= EEDYRD;
                  }
                break;
      case  11: // Change HostName, save on global the name then start sequence
                strlcpy(NewHost,(char*)txt,sizeof(NewHost));
                HostChg = 1;
                break;
      case  90: // Mcp Services command
                return(Mcp.Parse((char*)txt));
                break;
    }
return(false);
}
...
void setup(void) 
{
  ...
  // at boot write its host name
  Serial.println(); Serial.println(Web.HostName);
  ...
  if ((Serial.available() >= 1) && Serial.readBytes(buf, sizeof(buf)))
    {
      a = b = 0;
        ...
        if (buf[0] == 'i') {Data.Clk.Min ++;     Data.Clk.Sec = 0;}
       else
        if (buf[0] == 'I') {Data.Clk.Min += 15; Data.Clk.Sec  = 0;}
       else
        if (buf[0] == 'd') {Data.Clk.Hour = 23; Data.Clk.Min  = 30; Data.Clk.Sec = 0;}
       else
        if (buf[0] == 'm') {Data.Clk.Day  = 31; Data.Clk.Hour = 23; Data.Clk.Min = 30; Data.Clk.Sec = 0;}
       else
  ...
  switch (Boot)
        {
          ...
          case 2:   // Init I2C devices then start cyclic timer (GPIO[0,2] used fr I2C an not available)
                    ...
                    Boot++; 
                    break;
          case 3:   // Read the day log (if saved) at power-on
                    EeCmd.Addr = DAYADDR + MONSIZE * ( Data.Clk.Mon -1) + DAYSIZE * (Data.Clk.Day -1); 
                    EeCmd.Data = &Log; 
                    EeCmd.Cmd  = EEDYRD;
                    Boot++;
                    break;
          case 4:    // act day data acquired, nxt
#ifndef   TRACING
                    Boot++;
 #endif                    
                    break;
...
//    Serial.printf("%hd/%hhd/%hd %hhd:%02hhd(%hd,%hd)\n",Data.Clk.Yday,Data.Clk.Mon,Data.Clk.Year + 1900,Data.Clk.Hour,Data.Clk.Min,Data.Mcp.Wh.Pi[0],Data.Mcp.Wh.Po[0]);
   ...
   switch (HostChg)
        {
          case  1:  // save new name on ws.ini
                    ini = new IniFile("ws.ini");
                    ini->WriteString("misc","hostname",NewHost);
                    Serial.printf("\n New Host Name is: \n%s\n",NewHost);
                    delete ini;
                    HostChg++;
                    break;
          case   2: // wait a little to save name on file
                    delay(500);
                    HostChg++;
                    break;
          case   3: // Reboot machine
                    ESP.restart();
        }
  // EatTime loop
  delay(50);
